...
import { GET_AGENT_BY_PATH } from '@/lib/queries'
import { DrupalAgent } from '@/lib/types'
...
interface AgentByPathData {
  route: {
    entity: DrupalAgent
  } | null
}

async function getAgent(path: string): Promise<DrupalAgent | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<AgentByPathData>({
      query: GET_AGENT_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/agents/${slug.join('/')}`
  const item = await getAgent(path)

  if (!item) {
    return { title: 'Agent Not Found | Real Estate' }
  }
