...
interface NeighborhoodCardProps {
  item: DrupalNeighborhood
}

export default function NeighborhoodCard({ item }: NeighborhoodCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      ...
      <div className="p-6">
          {(item as any).medianPrice && (
            <p className="text-sm text-slate-700 font-medium mb-2">{(item as any).medianPrice}</p>
          )}
