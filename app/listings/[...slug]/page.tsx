...
import { GET_LISTING_BY_PATH } from '@/lib/queries'
import { DrupalListing } from '@/lib/types'
...
interface ListingByPathData {
  route: {
    entity: DrupalListing
  } | null
}
...
async function getListing(path: string): Promise<DrupalListing | null> {
  try {
    ...
    const { data } = await apolloClient.query<ListingByPathData>({
      query: GET_LISTING_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const item = await getListing(path)

  if (!item) {
    return { title: 'Listing Not Found | Real Estate' }
  }
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).squareFeet && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).squareFeet}</dd>
                    </div>
                  )}
                  {(item as any).address && (
                    <div>
                  ...
                  {(item as any).propertyType && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).propertyType}</dd>
                    </div>
                  )}
                  {(item as any).listingType && (
                    <div>
                      <dt className="text-sm text-gray-500">Listing Type</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).listingType}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
