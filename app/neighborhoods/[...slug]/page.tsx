...
import { GET_NEIGHBORHOOD_BY_PATH } from '@/lib/queries'
import { DrupalNeighborhood } from '@/lib/types'
...
interface NeighborhoodByPathData {
  route: {
    entity: DrupalNeighborhood
  } | null
}

async function getNeighborhood(path: string): Promise<DrupalNeighborhood | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<NeighborhoodByPathData>({
      query: GET_NEIGHBORHOOD_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/neighborhoods/${slug.join('/')}`
  const item = await getNeighborhood(path)

  if (!item) {
    return { title: 'Neighborhood Not Found | Real Estate' }
  }

  return {
    title: `${item.title} | Real Estate`,
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).medianPrice && (
                    <div>
                      <dt className="text-sm text-gray-500">Median Price</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).medianPrice}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
